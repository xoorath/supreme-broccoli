...
constexpr unsigned Get(const char *str, unsigned len, unsigned hash = 0xffffffff)
{
    return len > 0 ? Get(str + 1, len - 1, table[((hash) ^ ((unsigned char)*str)) & 0xff] ^ ((hash) >> 8)) : hash;
}
...
constexpr unsigned Get(const char *str, unsigned len) {
    return ~Internal::Get(str, len);
}

constexpr unsigned Get(const char *str) {
    return ~Internal::Get(str, Internal::StrLen(str));
}
}